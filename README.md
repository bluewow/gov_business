...
**수집은 수동 실행만 지원한다.** 자동 스케줄러도, 이를 위한 HTTP 엔드포인트도 두지 않았다 — 유료 API 호출이 모르는 사이에 도는 것을 막기 위해서다. STEP 2 화면의 버튼이나 `pnpm ingest` 로 직접 돌린다.
...
터미널 실행(`pnpm ingest`, `pnpm db:embed`)에는 이쪽이 필요하다. 읽는 우선순위는 **브라우저 키 > `.env`** 라 둘을 같이 써도 된다.
