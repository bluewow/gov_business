...
- **토큰 사용량은 응답의 `usage` 를 그대로 기록한다** — OpenAI 응답에 늘 들어오는데 그동안 5개 호출 지점 전부 버리고 있었고, 그래서 절감 논의가 전부 추정이었다. [ai-usage.ts](src/lib/ai-usage.ts) 의 `recordAiUsage()` 를 호출 직후에 부르면 `ai_usage` 에 한 줄 쌓이고 「설정 → API 키」에서 기능별로 볼 수 있다. **이 함수는 절대 throw 하지 않는다** — 사용량 기록 실패로 검토·초안이 죽으면 본말전도다. 임베딩은 배치라 `items` 에 건수를 따로 남긴다. **새 LLM 호출을 추가하면 `recordAiUsage()` 도 같이 붙일 것** — 안 붙이면 그 기능만 조용히 집계에서 빠진다.
- **초안은 첨부 원문 대신 검토·전략의 요약을 쓴다** — 초안은 PSST 섹션마다 호출되므로, 같은 공고문 16,000자를 매번 보내면 4섹션에 64,000자가 나간다. 그 내용은 이미 검토(자격요건 판정)와 전략(평가 관점·전략 포인트)이 한 번 읽고 정리해 DB 에 넣어 둔 것이다. `buildAnnouncementBrief()` 가 그 둘을 합쳐 넘기고, 첨부 원문은 요약이 있을 때 4,000자 발췌로만 붙인다(없으면 16,000자 그대로). 새 LLM 호출을 만들 때도 "이미 뽑아 둔 결과가 있는가" 를 먼저 볼 것.
...
- **수집·임베딩은 백그라운드 실행** — 화면 버튼은 advisory lock 을 응답 전에 잡고 `after()` 에 작업을 넘긴다(페이지를 옮겨도 계속). 실행이 도중에 죽으면 잠금은 자동 해제되지만 `ingestion_runs` 의 RUNNING 은 남는데, `listIngestionRuns()` 가 "잠금이 비어 있는데 RUNNING" 인 기록을 FAILED 로 자동 정리한다. 외부 fetch 에는 전부 타임아웃(30~60초)이 걸려 있다 — 정부 API 가 매달리면 수집이 영영 안 끝나기 때문.
